...
// create_output_wave(ft, particles, particle_coefs)
// SaveParamfile(out_fortran)

// window_edge_checking(out)
// cut_ft_smaller_for_isosurface(dat2)
// create_windowed_spots(ft, out, rr)
// create_unwindowed_spots(ft, out, rr)
// AppendGvecsToVk(ft, out, kexp, vkexp)

function x2pnt3d(w,xx,yy,zz)
	wave w
	variable xx,yy,zz
	variable ii,jj,kk
	ii = (xx - dimoffset(w,0))/dimdelta(w,0)
	jj = (yy - dimoffset(w,1))/dimdelta(w,1)
	kk = (zz - dimoffset(w,2))/dimdelta(w,0)
	make/o/n=3 x2pnt_results
	x2pnt_results[0] = ii
	x2pnt_results[1] = jj
	x2pnt_results[2] = kk
	return x2pnt_results
end function

function cart2sph(xx,yy,zz)
	variable xx,yy,zz
	variable rho, phi, theta
	make/o/n=3 scoords
	rho = sqrt(xx*xx + yy*yy + zz*zz)
	phi = acos(zz/rho)
	theta = asin(yy/(rho*sin(phi)))
	scoords[0] = rho
	scoords[1] = phi
	scoords[2] = theta
	return scoords
...
function sph2cart(rho,phi,theta)
	variable rho,phi,theta
	variable xx,yy,zz
	make/o/n=3 ccoords
	xx = rho*sin(phi)*cos(theta)
	yy = rho*sin(phi)*sin(theta)
	zz = rho*cos(phi)
	ccoords[0] = xx
	ccoords[1] = yy
	ccoords[2] = zz
...
function ft_to_spherical(ft)
	wave ft
	duplicate/o ft, ft_sph
	variable phi_min, phi_max, rho_min, rho_max, theta_min, theta_max
	variable zmax, zmin, ymax, ymin, xmax, xmin
	zmax = (dimoffset(ft,2)+dimsize(ft,2)*dimdelta(ft,2))
	zmin = dimoffset(ft,2)
	rho_min = 0.0 //dimdelta(ft,0)
	rho_max = sqrt( (dimoffset(ft,0) + dimsize(ft,0)*dimdelta(ft,0))^2 + (dimoffset(ft,1) + dimsize(ft,1)*dimdelta(ft,1))^2 + (dimoffset(ft,2) + dimsize(ft,2)*dimdelta(ft,2))^2 )
	phi_min = 0.0 //acos( zmax/rho_min )
	phi_max = acos( zmin/rho_max )
	theta_min = asin( dimoffset(ft,1)/(rho_max*sin(phi_max)) )
	theta_max = pi/2 //asin( (dimoffset(ft,1)+dimsize(ft,1)*dimdelta(ft,1))/(rho_min*sin(phi_min)) )
	setscale/I x, rho_min, rho_max, ft_sph
	setscale/I y, phi_min, phi_max, ft_sph
	setscale/I z, theta_min, theta_max, ft_sph
	wave ccoords
	variable i,j,k,  rho,phi,theta
	for(i=0; i<dimsize(ft_sph,0); i+=1)
		print i
		for(j=0; j<dimsize(ft_sph,1); j+=1)
			for(k=0; k<dimsize(ft_sph,2); k+=1)
				rho = (dimoffset(ft_sph,0)+i*dimdelta(ft_sph,0))
				phi = (dimoffset(ft_sph,1)+j*dimdelta(ft_sph,1))
				theta = (dimoffset(ft_sph,2)+k*dimdelta(ft_sph,2))
				sph2cart(rho,phi,theta)
				ft_sph[i][j][k] = ft(ccoords[0])(ccoords[1])(ccoords[2])
			endfor
		endfor
	endfor
end function

function IntegratedIntensity(ft,out,iso_av)
	wave ft, out, iso_av
	variable i,j,k,n
	make/o/n=20 sumint, avg_width, avg_background
	sumint = 0
	for(n=0; n<20; n+=1)
		avg_width[n] = ( (out[n][1]-out[n][0]) + (out[n][3]-out[n][2]) + (out[n][5]-out[n][4]) ) /3.0*dimdelta(ft,0)
	for(i=out[n][0]; i<=out[n][1]; i+=1)
	for(j=out[n][2]; j<=out[n][3]; j+=1)
	for(k=out[n][4]; k<=out[n][5]; k+=1)
		sumint[n] += ft[i][j][k]
	endfor
	endfor
	endfor
	for(i=x2pnt(iso_av, out[n][10]-avg_width[n]/2.0); i<= x2pnt(iso_av, out[n][10]+avg_width[n]/2.0); i+=1)
		avg_background[n] += iso_av[i]
	endfor
	print n
	//print avg_width[n]
	//print x2pnt(iso_av, out[n][10]-avg_width[n]/2.0)
	//print x2pnt(iso_av, out[n][10]+avg_width[n]/2.0)
	avg_background[n] /= (x2pnt(iso_av, out[n][10]+avg_width[n]/2.0)-x2pnt(iso_av, out[n][10]-+avg_width[n]/2.0))
	print sumint[n]
	print avg_background[n]
	sumint[n] /= avg_background[n]
	endfor
	return sumint
end function
...
function cut_ft_smaller_for_isosurface(ft)
	...
	setscale /P x, dimoffset(ft,0)/2, dimdelta(ft,0), ft_iso
	setscale /P y, dimoffset(ft,2)/2, dimdelta(ft,1), ft_iso
	setscale /P z, dimoffset(ft,2)/2, dimdelta(ft,2), ft_iso
	variable i,j,k
...
function get_points_for_spot_CM(ft_iso, thresh)
// Sets all values of fiso that are below thresh to 0
// Puts coordinates of points that are non-zero inside cluster_points for FPClustering
	wave ft_iso
	variable thresh
	make/o/n=(256,256,256) ft_iso_thresholded
	variable i,j,k, numpoints
	ft_iso_thresholded = ft_iso
	
	Make/O /N=(3,1) cluster_points
	numpoints = 0
	for(i=0; i<256; i+=1)
		printf "%g\r", i
		for(j=0; j<256; j+=1)
			...
			for(k=0; k<256; k+=1)
				//printf "    %g\r", k
				if( ft_iso[i][j][k] < thresh)
					ft_iso_thresholded[i][j][k] = 0
				else
					if(numpoints == 0)
						cluster_points[0][0] = dimdelta(ft_iso,0)*i + dimoffset(ft_iso,0)
						cluster_points[1][0] = dimdelta(ft_iso,1)*j + dimoffset(ft_iso,1)
						cluster_points[2][0] = dimdelta(ft_iso,2)*k + dimoffset(ft_iso,2)
						numpoints = numpoints + 1
					else
						InsertPoints /M=1 dimsize(cluster_points,1), 1, cluster_points
						numpoints = numpoints + 1
						cluster_points[0][numpoints] = dimdelta(ft_iso,0)*i + dimoffset(ft_iso,0)
						cluster_points[1][numpoints] = dimdelta(ft_iso,1)*j + dimoffset(ft_iso,1)
						cluster_points[2][numpoints] = dimdelta(ft_iso,2)*k + dimoffset(ft_iso,2)
					endif
				endif
			endfor
		endfor
	endfor
	//MatrixOP/O ddt=cluster_points^t
	//FPClustering /CM /INCD cluster_points
end
